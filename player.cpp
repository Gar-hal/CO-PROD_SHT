		...
		if (g_Player[i].use == TRUE)
		{
			...
			// キー入力で移動 
			{
				...
				if (g_Player[i].jump == TRUE)
				{
					...
					float y = PLAYER_JUMP_Y_MAX * cosf(XM_PI / 2 + angle);
					g_Player[i].jumpY = y; // なんでY座標に直接足してないのか
					// →影も動いちゃうから、.jumpYの値分描画時に上に描画する
				...
				// 移動が終わったらエネミーとの当たり判定
				{
					...
					for (int j = 0; j < ENEMY_MAX; j++)
					{
						...
						if (enemy[j].use == TRUE)
						{
							BOOL ans = AxisAlignedBoundingBox(g_Player[i].pos, g_Player[i].w, g_Player[i].h,
