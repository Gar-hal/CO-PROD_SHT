...
// game pad用設定値
//#define DEADZONE		1000			// 各軸の25%を無効ゾーンとする
...
void UpdatePad(void)
{
	...
	for (i = 0; i < padCount; i++)
	{
		...
		//* Rz-axis (forward)
		if (dijs.lRz < 25000)							padState[i] |= BUTTON_R_UP;
		//* Rz-axis (backward)
		if (dijs.lRz > 40000)							padState[i] |= BUTTON_R_DOWN;
		//* z-axis (left)
		if (dijs.lZ < 20000)							padState[i] |= BUTTON_R_LEFT;
		//* z-axis (right)
		if (dijs.lZ > 45000)							padState[i] |= BUTTON_R_RIGHT;
		...
		//* SHAREボタン
		if (dijs.rgbButtons[rgbButtons_SELECT] & 0x80)	padState[i] |= BUTTON_SHARE;
		//* OPTIONSボタン
		if (dijs.rgbButtons[rgbButtons_START] & 0x80)	padState[i] |= BUTTON_OPTIONS;
		//* Ｌ３押し込み
		if (dijs.rgbButtons[rgbButtons_L3] & 0x80)	padState[i] |= BUTTON_L3;
		//* Ｒ３押し込み
		if (dijs.rgbButtons[rgbButtons_R3] & 0x80)	padState[i] |= BUTTON_R3;
		//* ＰＳボタン
		if (dijs.rgbButtons[rgbButtons_PS] & 0x80)	padState[i] |= BUTTON_PS;
		//* トラックパッド
		if (dijs.rgbButtons[rgbButtons_PAD] & 0x80)	padState[i] |= BUTTON_PAD;
